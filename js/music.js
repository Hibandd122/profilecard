...
function musicRenderLoop() {
    ...
    if (card) {
        card.style.boxShadow = `0 15px 40px -10px rgba(0,0,0,0.5), 0 0 ${10 + Math.floor(intensity*10)}px var(--glow-primary)`;
    }
    ...
    if (avatarImg && !avatarImg.classList.contains('active-touch')) {
        ...
        avatarImg.style.transform = `translateX(-50%) scale(${scale.toFixed(3)})`;
        avatarImg.style.filter = `drop-shadow(0 0 ${20 + Math.floor(intensity*20)}px var(--glow-secondary))`;
    }
}
if (visualizer) musicRenderLoop();